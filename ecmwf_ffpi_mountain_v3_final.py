...
# =========================================================
# ECMWF Experimental FFPI - Mountain V3 FINAL
# Wide Middle East map + North Oman diagnostics
# Robust plotting: no Cartopy gridliner, with plain fallback
# =========================================================

OUTDIR = Path("output")
OUTDIR.mkdir(exist_ok=True)

GRIB_FILE = OUTDIR / "ecmwf_ffpi_mountain_v3_final_0_72h.grib2"
...
# Broad box includes Al Hajar / Bidbid surroundings
# =========================================================
...
oman_tp = subset_box(
    tp72, OMAN_LAT_MIN, OMAN_LAT_MAX, OMAN_LON_MIN, OMAN_LON_MAX
)
oman_rate = subset_box(
    max_rate, OMAN_LAT_MIN, OMAN_LAT_MAX, OMAN_LON_MIN, OMAN_LON_MAX
)
oman_ro = subset_box(
    runoff72, OMAN_LAT_MIN, OMAN_LAT_MAX, OMAN_LON_MIN, OMAN_LON_MAX
)
oman_ffpi = subset_box(
    ffpi_clean, OMAN_LAT_MIN, OMAN_LAT_MAX, OMAN_LON_MIN, OMAN_LON_MAX
)

diag = (
    "ECMWF Mountain V3 FINAL - North Oman Diagnostic\n"
    f"IFS Run: {run_time.strftime('%d %b %Y %H UTC')}\n"
    f"Box: {OMAN_LAT_MIN}-{OMAN_LAT_MAX}N, "
    f"{OMAN_LON_MIN}-{OMAN_LON_MAX}E\n"
...
# Key cities for flash-flood monitoring
cities = {
    "Jeddah": (39.19, 21.49),
    "Makkah": (39.86, 21.39),
    "Taif": (40.42, 21.27),
...
def save_plain_fallback():
    print("Creating plain matplotlib fallback map...")
    ...
    for name, (lon, lat) in cities.items():
        ...
        ax.text(lon + 0.35, lat + 0.25, name, fontsize=9)
    ...
    ax.set_title(
        ...
        "Mountain V3 FINAL | robust fallback view",
        fontsize=14,
        weight="bold",
    )
...
try:
    ...
    # IMPORTANT: no Cartopy gridliner
    ...
    plt.title(
        "Unbiased Experimental FFPI - ECMWF | Middle East\n"
        f"IFS Run: {run_text} | Forecast: +3 to +72 h\n"
        "Mountain V3 FINAL | wide view | North Oman diagnostic",
        fontsize=14,
        weight="bold",
    )
    ...
    try:
        plt.savefig(MAP_FILE, dpi=180, bbox_inches="tight")
        plt.close()
        print("Cartopy map saved successfully.")
    except Exception as e:
        print(f"Cartopy save failed: {e}")
        plt.close()
        save_plain_fallback()

except Exception as e:
    print(f"Cartopy map setup failed: {e}")
    save_plain_fallback()

print("----------------------------------------")
print("ECMWF Mountain V3 FINAL completed successfully")
